...
if __name__ == '__main__':
    ...
    for dataset in benchmark.keys():
        ...
        os.makedirs("datasets_random/{0}".format(dataset), exist_ok=True)

        logdf = log_to_dataframe(f'./logs/{setting["log_file"]}', setting['log_format'])
        logdf.to_csv(f"datasets_random/{setting['log_file']}_structured.csv")
        labelled_logs = pd.read_csv(f'./logs_groundtruth_v2/{dataset}_2k.log_structured.csv')
        test_samples = [(row['Content'], row['EventTemplate']) for _, row in labelled_logs.iterrows()]
        template_dict = {k: v for (k, v) in test_samples}
        with open("datasets_random/{0}/test.json".format(dataset), "w") as f:
